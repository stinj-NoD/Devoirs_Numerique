...
- `.claude/agents/security-auditor.md` — agent d'audit de sécurité/robustesse (injection DOM, échappement, stockage local, service worker) ; diagnostique uniquement et régénère un rapport daté `docs/security-audit-<date>.md`, n'applique aucun correctif
- `.claude/agents/ui-craftsman.md` — agent d'interface et de rendu visuel cross-plateforme (responsive, thèmes clair/sombre, tactile/clavier) ; corrige directement `css/app.css` et `js/ui*.js`
