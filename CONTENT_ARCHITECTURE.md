...
| Niveau | Matière | Sous-thème | Leçons | Exercices |
|---|---|---|---|---|
| CP | Français | Phrase et ponctuation `(cp-phrase-ponctuation-subtheme)` | 2 | 0 |
| CP | Questionner l'espace | Plan et quartier `(cp-plan-quartier-subtheme)` | 2 | 0 |
| CE2 | Français | Lecture et vocabulaire `(ce2-lecture-vocabulaire-subtheme)` | 3 | 0 |
| CM1 | Français | Lecture et vocabulaire `(cm1-francais-lecture-vocabulaire-subtheme)` | 3 | 0 |
| CM2 | Histoire | Récits historiques `(cm2-histoire-recits-subtheme)` | 3 | 0 |
| CM2 | EMC | Institutions et débat `(cm2-emc-institutions-debat-subtheme)` | 3 | 0 |
...
- **Exercices** (5) :
  ...
  - Trouver le nombre caché `(add-3)` — engine: `math-input`, type=add-trou, questions=5
...
- **Exercices** (3) :
  - Retirer jusqu'à 10 `(sub-1)` — engine: `math-input`, type=sub-simple, questions=5
  - Retirer jusqu'à 20 `(sub-2)` — engine: `math-input`, type=sub-simple, questions=5
  - Retirer jusqu'à 30 `(sub-3)` — engine: `math-input`, type=sub-simple, questions=5
...
- **Exercices** (5) :
  - Compter `(math-count-1)` — engine: `counting`, questions=5
  - Compter plus loin `(math-count-2)` — engine: `counting`, questions=5
  - Comparer jusqu'à 10 `(comp-1)` — engine: `choice-engine`, questions=5, range=10
  - Comparer jusqu'à 20 `(comp-2)` — engine: `choice-engine`, questions=5, range=20
  - Comparer jusqu'à 50 `(comp-3)` — engine: `choice-engine`, questions=5, range=50
...
- **Exercices** (4) :
  - L'Oiseau Rapide `(oiseau-1)` — engine: `math-input`, type=oiseau-math, questions=5, vitesse=8
  - Carré Magique 1 `(carre-1)` — engine: `math-input`, type=carre-somme, questions=5
  - Carré Magique 2 `(carre-2)` — engine: `math-input`, type=carre-somme, questions=5
  - Carré Magique 3 `(carre-3)` — engine: `math-input`, type=carre-somme, questions=5
...
- **Exercices** (3) :
  - Petits Nombres `(cp-dictee-0-10)` — engine: `math-input`, type=number-spelling, questions=5
  - Nombres de 10 à 20 `(cp-dictee-10-20)` — engine: `math-input`, type=number-spelling, questions=5
  - Nombres de 20 à 69 `(cp-dictee-20-69)` — engine: `math-input`, type=number-spelling, questions=5
...
- **Exercices** (2) :
  - Un ou Une ? `(cp-un-une)` — engine: `choice-engine`, type=gender-articles, category=gender_cp, questions=5, options=["un","une"]
  - Le ou La ? `(cp-le-la)` — engine: `choice-engine`, type=gender-articles, category=gender_cp, questions=5, options=["le","la"]
...
- **Exercices** (9) :
  - Compléments à 100 `(add-100)` — engine: `math-input`, type=add-trou, questions=10
  - Calcul rapide `(add-chrono)` — engine: `math-input`, type=oiseau-math, questions=10, vitesse=5
  - Comparer jusqu'à 100 `(comp-ce1-1)` — engine: `choice-engine`, questions=10, range=100
  - Le Marché `(bank-ce1-1)` — engine: `math-input`, type=cibles, questions=5
  - Carré Expert `(carre-ce1-1)` — engine: `math-input`, type=carre-somme, questions=5
  - Additions jusqu'à 20 `(ce1-additions-20)` — engine: `math-input`, type=add-simple, questions=10
  - Soustractions jusqu'à 20 `(ce1-soustractions-20)` — engine: `math-input`, type=sub-simple, questions=10
  - Additions jusqu'à 50 `(ce1-additions-50)` — engine: `math-input`, type=add-simple, questions=10
  - Comparer jusqu'à 200 `(ce1-comp-200)` — engine: `choice-engine`, questions=10, range=200
...
- **Exercices** (10) :
  ...
  - A ou à? `(ce1-homo-aa)` — engine: `choice-engine`, type=homophone-duel, category=a_à, questions=10, choices=["a","à"]
  - Et ou est? `(ce1-homo-et)` — engine: `choice-engine`, type=homophone-duel, category=et_est, questions=10, choices=["et","est"]
  ...
  - Son ou Sont? `(ce1-homo-son)` — engine: `choice-engine`, type=homophone-duel, category=son_sont, questions=10, choices=["son","sont"]
...
- **Exercices** (6) :
  - Un ou Une? `(ce1-gram-un-une)` — engine: `choice-engine`, type=gender-articles, category=gender_ce1_classe, questions=8, options=["un","une"]
  - Le, La ou L'? `(ce1-gram-le-la)` — engine: `choice-engine`, type=gender-articles, category=gender_ce1_maison, questions=8, options=["le","la","l'"]
  - Les noms de la nature `(ce1-gram-nature)` — engine: `choice-engine`, type=gender-articles, category=gender_ce1_nature, questions=8, options=["le","la","l'"]
...
- **Exercices** (7) :
  - Les Indispensables `(ce1-conj-base)` — engine: `conjugation`, category=etre_avoir_p, questions=5, tenses="présent", verbs=["être","avoir"]
  - Verbes en -ER (1) `(ce1-conj-er-facile)` — engine: `conjugation`, category=present_1, questions=5, tenses="présent", verbs=["chanter","jouer","parler","marcher","aimer","dessiner"]
  - Le verbe ALLER `(ce1-conj-aller)` — engine: `conjugation`, category=present_3_freq, questions=5, tenses="présent", verbs="aller"
  - Verbes en -ER (2) `(ce1-conj-er-action)` — engine: `conjugation`, category=present_1, questions=5, tenses="présent", verbs=["cacher","grimper","sauter","tomber","chercher","trouver"]
  - Le Cas du 'Nous' `(ce1-conj-er-regles)` — engine: `conjugation`, category=present_1, questions=6, tenses="présent", verbs=["manger","nager","ranger","lancer","placer","voyager"]
  - Défi Verbe-o-tron `(ce1-conj-expert)` — engine: `conjugation`, category=present_1, questions=10
  - Être et avoir (2) `(ce1-conj-etre-avoir-plus)` — engine: `conjugation`, category=etre_avoir_p, questions=8, tenses="présent", verbs=["être","avoir"]
...
- **Exercices** (9) :
  ...
  - Comparer < 1000 `(ce2-comp-1000-compare)` — engine: `choice-engine`, questions=10, range=1000
...
- **Exercices** (18) :
  - Et ou Est ? `(ce2-h-et)` — engine: `choice-engine`, type=homophone-duel, category=et_est, questions=8, choices=["et","est"]
  - A ou à ? `(ce2-h-a)` — engine: `choice-engine`, type=homophone-duel, category=a_à, questions=8, choices=["a","à"]
  - Son ou Sont ? `(ce2-h-son)` — engine: `choice-engine`, type=homophone-duel, category=son_sont, questions=8, choices=["son","sont"]
  - On ou Ont ? `(ce2-h-on)` — engine: `choice-engine`, type=homophone-duel, category=on_ont, questions=8, choices=["on","ont"]
  - Ou ou Où ? `(ce2-h-ou)` — engine: `choice-engine`, type=homophone-duel, category=ou_où, questions=8, choices=["ou","où"]
  - Ce ou Se ? `(ce2-h-ce)` — engine: `choice-engine`, type=homophone-duel, category=ce_se, questions=8, choices=["ce","se"]
  - Ces ou Ses ? `(ce2-h-ces)` — engine: `choice-engine`, type=homophone-duel, category=ces_ses, questions=8, choices=["ces","ses"]
  - Un ou Une ? `(ce2-g-un-une)` — engine: `choice-engine`, type=gender-articles, category=gender_ce2_objets, questions=8, options=["un","une"]
  - Le, La ou L' ? `(ce2-g-le-la)` — engine: `choice-engine`, type=gender-articles, category=gender_ce2_elision, questions=8, options=["le","la","l'"]
  - Articles malins `(ce2-g-mixte)` — engine: `choice-engine`, type=gender-articles, category=gender_ce2_mixte, questions=8, options=["le","la","l'"]
  ...
  - Un ou Une ? (objets) `(ce2-g-objets)` — engine: `choice-engine`, type=gender-articles, category=gender_ce2_objets, questions=8, options=["un","une"]
  - Le, La ou L' ? (2) `(ce2-g-elision)` — engine: `choice-engine`, type=gender-articles, category=gender_ce2_elision, questions=8, options=["le","la","l'"]
...
- **Exercices** (10) :
  - Rappel : Le Présent `(ce2-conj-pres-all)` — engine: `conjugation`, category=present_1, questions=10, tenses="présent"
  - Futur : Être et Avoir `(ce2-conj-etre-avoir-futur)` — engine: `conjugation`, category=etre_avoir_f, questions=6, tenses="futur", verbs=["être","avoir"]
  - Futur : Verbes en -ER `(ce2-conj-futur-er)` — engine: `conjugation`, category=future_1, questions=8, tenses="futur"
  - Imparfait : Auxiliaires `(ce2-conj-imp-ea)` — engine: `conjugation`, category=etre_avoir_imp, questions=6, tenses="imparfait"
  - Imparfait : Verbes en -ER `(ce2-conj-imp-er)` — engine: `conjugation`, category=imparfait_1, questions=8, tenses="imparfait"
  - Défi : Présent ou Futur ? `(ce2-conj-defi-temps)` — engine: `conjugation`, category=future_1, questions=10, tenses=["présent","futur"]
  - Le 3ème groupe (1) `(ce2-conj-p3-freq)` — engine: `conjugation`, category=present_3_ce2_a, questions=8, tenses="présent"
  - Le 3ème groupe (2) `(ce2-conj-p3-pouvoir)` — engine: `conjugation`, category=present_3_ce2_b, questions=8, tenses="présent"
  - Présent : être et avoir `(ce2-conj-present-ea)` — engine: `conjugation`, category=etre_avoir_p, questions=8, tenses="présent", verbs=["être","avoir"]
  - Présent : 2e groupe `(ce2-conj-present-2g)` — engine: `conjugation`, category=present_2, questions=8, tenses="présent"
...
- **Exercices** (15) :
  - Grands Nombres `(cm1-m-big)` — engine: `math-input`, type=dictée-nombres, questions=5
  - Fractions `(cm1-frac-1)` — engine: `math-input`, type=fraction-view, questions=10
  ...
  - Lire des fractions `(cm1-frac-2)` — engine: `math-input`, type=fraction-view, questions=10
  - Trés grands nombres `(cm1-m-big-2)` — engine: `math-input`, type=dictée-nombres, questions=5
  - Fractions plus précises `(cm1-frac-3)` — engine: `math-input`, type=fraction-view, questions=10
  - Grands nombres (3) `(cm1-m-big-3)` — engine: `math-input`, type=dictée-nombres, questions=5
...
- **Exercices** (10) :
  - Chiffres romains `(cm1-romains)` — engine: `conversion`, questions=5
  ...
  - Chiffres romains (2) `(cm1-romains-50)` — engine: `conversion`, questions=5
  - Minutes vers secondes (2) `(cm1-heures-secondes)` — engine: `conversion`, questions=10
  - Chiffres romains (3) `(cm1-romains-100)` — engine: `conversion`, questions=5
...
- **Exercices** (5) :
  - Ce ou Se ? `(cm1-h-ce)` — engine: `choice-engine`, type=homophone-duel, category=ce_se, questions=10, choices=["ce","se"]
  - Ces ou Ses ? `(cm1-h-ces)` — engine: `choice-engine`, type=homophone-duel, category=ces_ses, questions=10, choices=["ces","ses"]
  - A ou À ? `(cm1-h-aa)` — engine: `choice-engine`, type=homophone-duel, category=a_à, questions=10, choices=["a","à"]
  - Son ou Sont ? `(cm1-h-son)` — engine: `choice-engine`, type=homophone-duel, category=son_sont, questions=10, choices=["son","sont"]
  - On ou Ont ? `(cm1-h-on)` — engine: `choice-engine`, type=homophone-duel, category=on_ont, questions=10, choices=["on","ont"]
...
- **Exercices** (8) :
  - L'Imparfait `(cm1-c-imp)` — engine: `conjugation`, category=imparfait_1, questions=10, tenses="imparfait"
  - Passé Composé (1) `(cm1-c-pc-avoir)` — engine: `conjugation`, category=pc_1, questions=5, tenses="passé composé", verbs=["manger","chanter","jouer","chercher","trouver"]
  - Passé Composé (2) `(cm1-c-pc-etre)` — engine: `conjugation`, category=pc_1, questions=5, tenses="passé composé", verbs=["tomber","monter","arriver","entrer","rester"]
  - Passé Composé (Mix) `(cm1-c-pc-mix)` — engine: `conjugation`, category=pc_1, questions=10, tenses="passé composé"
  - Passé Composé (Expert) `(cm1-c-pc-3)` — engine: `conjugation`, category=pc_3_cm1, questions=10, tenses="passé composé"
  - Le 2ème groupe `(cm1-conj-ir)` — engine: `conjugation`, category=present_2, questions=8, tenses="présent"
  - Le futur simple `(cm1-c-futur)` — engine: `conjugation`, category=future_1, questions=10, tenses="futur"
  - Être et avoir à l'imparfait `(cm1-c-ea-imp)` — engine: `conjugation`, category=etre_avoir_imp, questions=8, tenses="imparfait", verbs=["être","avoir"]
...
- **Exercices** (2) :
  - Un ou Une ? `(cm1-g-un-une)` — engine: `choice-engine`, type=gender-articles, category=gender_cm1_scolaire, questions=8, options=["un","une"]
  - Le, La ou L' ? `(cm1-g-le-la)` — engine: `choice-engine`, type=gender-articles, category=gender_cm1_elision, questions=8, options=["le","la","l'"]
...
- **Exercices** (10) :
  - Frise historique `(cm1-histoire-frise-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=order, timelineId=cm1-reperes-celebres
  - Placer sur la frise `(cm1-histoire-frise-place)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=place, timelineId=cm1-place-moyen-age
  - Placer sur la frise - Niv 1 `(cm1-histoire-frise-place-n1)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=place, timelineId=cm1-place-moyen-age
  - Placer sur la frise - Niv 2 `(cm1-histoire-frise-place-n2)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=place, timelineId=cm1-place-moyen-age
  - Placer sur la frise - Niv 3 `(cm1-histoire-frise-place-n3)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=place, timelineId=cm1-place-moyen-age
  - Frise : Préhistoire `(cm1-histoire-frise-prehistoire-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=order, timelineId=cm1-ordre-prehistoire-1
  - Frise : Antiquité `(cm1-histoire-frise-antiquite-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=order, timelineId=cm1-antiquite-ordre-1
  - Frise : Temps modernes `(cm1-histoire-frise-modernes-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=order, timelineId=cm1-ordre-modernes-2
  - Placer : Antiquité `(cm1-histoire-frise-antiquite-place)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=place, timelineId=cm1-place-antiquite
  - Placer : Temps modernes `(cm1-histoire-frise-modernes-place)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm1, mode=place, timelineId=cm1-place-modernes
...
- **Exercices** (7) :
  - Les Milliards `(cm2_grands_nombres)` — engine: `math-input`, type=dictée-nombres, questions=10
  - Division Mentale `(cm2_division_mentale)` — engine: `math-input`, type=calc-mental, questions=10
  - Tables x11 à x15 `(cm2_tables_x)` — engine: `math-input`, type=calc-mental, questions=10, range=[11,15]
  - Tables x16 à x20 `(cm2_tables_x_16_20)` — engine: `math-input`, type=calc-mental, questions=10, range=[16,20]
  - Très grands nombres `(cm2_grands_nombres_2)` — engine: `math-input`, type=dictée-nombres, questions=10
...
- **Exercices** (5) :
  ...
  - Fractions `(cm2_fractions_lecture)` — engine: `math-input`, type=fraction-view, questions=10
  ...
  - Fractions (2) `(cm2_fractions_lecture_2)` — engine: `math-input`, type=fraction-view, questions=10
...
- **Exercices** (9) :
  ...
  - Chiffres Romains `(cm2-romains-100)` — engine: `conversion`, questions=6
  ...
  - Chiffres romains (3) `(cm2-romains-500)` — engine: `conversion`, questions=6
...
- **Exercices** (7) :
  - Présent 3e Groupe `(cm2_conj_present_3)` — engine: `conjugation`, category=present_3_freq, questions=10, tenses="présent"
  - Futur Simple `(cm2_conj_future)` — engine: `conjugation`, category=future_1, questions=10, tenses="futur"
  - L'Imparfait `(cm2_conj_imparfait)` — engine: `conjugation`, category=imparfait_1, questions=10, tenses="imparfait"
  - Passé Composé `(cm2_conj_pc)` — engine: `conjugation`, category=pc_1, questions=10, tenses="passé composé"
  - Passé Composé du 3e groupe `(cm2_conj_pc_3)` — engine: `conjugation`, category=pc_3_cm2, questions=10, tenses="passé composé"
  - Présent du 2e groupe `(cm2-conj-present-2g)` — engine: `conjugation`, category=present_2, questions=10, tenses="présent"
  - Être et avoir à l'imparfait `(cm2-conj-ea-imp)` — engine: `conjugation`, category=etre_avoir_imp, questions=8, tenses="imparfait", verbs=["être","avoir"]
...
- **Exercices** (7) :
  ...
  - Ce ou se ? `(cm2_homophones_ce_se)` — engine: `choice-engine`, type=homophone-duel, category=ce_se, questions=10, choices=["ce","se"]
  - Ces ou ses ? `(cm2_homophones_ces_ses)` — engine: `choice-engine`, type=homophone-duel, category=ces_ses, questions=10, choices=["ces","ses"]
  - A ou à ? `(cm2_homophones_a_a)` — engine: `choice-engine`, type=homophone-duel, category=a_à, questions=10, choices=["a","à"]
  - Ou ou où ? `(cm2_homophones_ou_ou)` — engine: `choice-engine`, type=homophone-duel, category=ou_où, questions=10, choices=["ou","où"]
  - Son ou Sont ? `(cm2-homophones-son-sont)` — engine: `choice-engine`, type=homophone-duel, category=son_sont, questions=10, choices=["son","sont"]
  - On ou Ont ? `(cm2-homophones-on-ont)` — engine: `choice-engine`, type=homophone-duel, category=on_ont, questions=10, choices=["on","ont"]
...
- **Exercices** (7) :
  - Un ou Une ? `(cm2_g_abstrait)` — engine: `choice-engine`, type=gender-articles, category=gender_cm2_abstrait, questions=8, options=["un","une"]
  - Le, La ou L' ? `(cm2_g_elision)` — engine: `choice-engine`, type=gender-articles, category=gender_cm2_elision, questions=8, options=["le","la","l'"]
...
- **Exercices** (6) :
  ...
  - Consommation responsable `(cm2-sciences-consommation-responsable)` — engine: `choice-engine`, type=factual-qcm, category=cm2-consommation-responsable, dataFile=data/science_cm2.json, questions=6

#### Frises historiques `(cm2-histoire-frises)`

- **Leçons** (1) :
  - Quelques repères du XXe siècle `(cm2-lesson-reperes-xxe-siecle)` — blocs: paragraph, bullets, example, tip
- **Exercices** (10) :
  - Frise historique `(cm2-histoire-frise-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=order, timelineId=cm2-revolution-ordre
  - Placer sur la frise `(cm2-histoire-frise-place)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=place, timelineId=cm2-place-xx
  - Placer sur la frise - Niv 1 `(cm2-histoire-frise-place-n1)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=place, timelineId=cm2-place-xx
  - Placer sur la frise - Niv 2 `(cm2-histoire-frise-place-n2)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=place, timelineId=cm2-place-xx
  - Placer sur la frise - Niv 3 `(cm2-histoire-frise-place-n3)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=place, timelineId=cm2-place-xx
  - Frise : Révolution `(cm2-histoire-frise-revolution-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=order, timelineId=cm2-ordre-revolution-3
  - Frise : XIXe siècle `(cm2-histoire-frise-xix-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=order, timelineId=cm2-xix-siecle-ordre
  - Frise : Progrès techniques `(cm2-histoire-frise-progres-ordre)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=order, timelineId=cm2-sciences-societe-ordre
  - Placer : République `(cm2-histoire-frise-republique-place)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=place, timelineId=cm2-place-republique
  - Placer : Europe `(cm2-histoire-frise-europe-place)` — engine: `timeline`, dataFile=data/history_chrono.json, questions=5, grade=cm2, mode=place, timelineId=cm2-place-europe
...
- **Leçons** (1) :
  - Agir pour un développement durable `(cm2-lesson-developpement-durable)` — blocs: paragraph, example, bullets, tip
- **Exercices** (2) :
...
- **Leçons** (2) :
  - Protéger les milieux `(cm2-lesson-proteger-environnement)` — blocs: paragraph, bullets, tip
