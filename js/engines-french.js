const EnginesFrench = {
    conjugation(p, lib) {
        ...
        const requestedVerbs = Array.isArray(p.verbs)
            ? p.verbs.map(normalizeToken)
            : (p.verbs ? [normalizeToken(p.verbs)] : []);
        const filteredPool = requestedVerbs.length
        ...
        if (isCompound) {
            const auxData = lib.conjugation.etre_avoir_p.find((v) => normalizeToken(v.infinitive) === normalizeToken(verb.aux));
            const auxiliary = auxData ? auxData.full[cIdx] : "a";
            let pp = verb.pp;
            if (normalizeToken(verb.aux) === "etre") {
                if (pIdx === 3) pp += "e";
    ...
    reading(params, lib) {
        ...
        const question = SecurityUtils.escapeHtml((picked.question || picked.prompt || "Lis et choisis la bonne réponse.").toString().trim());
    ...
    grammarChoice(params, lib) {
        ...
        return {
            question: `<span class="small-question">${SecurityUtils.escapeHtml(rawQuestion)}</span>`,
            answer,
            inputType: "qcm",
            isVisual: false,
            data: { choices }
