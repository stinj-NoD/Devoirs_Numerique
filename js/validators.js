const Validators = {
    ...
    hasNoHtmlMarkup(value) {
        return typeof value === 'string' && !/<\/?[a-zA-Z!][^>]*>/.test(value);
    },

    isSafeLessonText(value) {
    ...
    validateLessonBlock(block) {
        ...
        if (type === 'paragraph') {
            return this.isSafeLessonText(block.text)
                ? { valid: true }
                : { valid: false, reason: 'bloc paragraph sans texte ou texte invalide.' };
        }

        if (type === 'example' || type === 'tip') {
            return this.isSafeLessonText(block.content)
                ? { valid: true }
                : { valid: false, reason: `bloc ${type} sans contenu ou contenu invalide.` };
        }

        if (type === 'bullets') {
            return Array.isArray(block.items) && block.items.length > 0 && block.items.every((item) => this.isSafeLessonText(item))
                ? { valid: true }
        ...
        if (type === 'mini-table') {
            const hasHeaders = Array.isArray(block.headers)
                ...
                && block.headers.every((item) => this.isSafeLessonText(item));
            const hasRows = Array.isArray(block.rows)
                && block.rows.length > 0
                && block.rows.every((row) =>
                    Array.isArray(row)
                    && row.length === block.headers.length
                    && row.every((cell) => this.isSafeLessonText(cell))
                );
    ...
    validateExerciseData(exercise, dataSet) {
        ...
        if (exercise.engine === 'matching') {
            return this.validateMatchingDataset(exercise, dataSet);
        }

        if (exercise.engine === 'word-order') {
            return this.validateWordOrderDataset(exercise, dataSet);
        }
    ...
    validateMatchingDataset(exercise, dataSet) {
        if (!this.isPlainObject(dataSet.categories)) {
            return { valid: false, reason: 'dataset appariement sans categories.' };
        }

        const pool = dataSet.categories[exercise.params.category];
        if (!Array.isArray(pool) || pool.length === 0) {
            return { valid: false, reason: `Catégorie d'appariement introuvable (${exercise.params.category}).` };
        }
    ...
    validateWordOrderDataset(exercise, dataSet) {
        if (!this.isPlainObject(dataSet.categories)) {
            return { valid: false, reason: 'dataset remise en ordre sans categories.' };
        }
