...
const UI = {
    ...
    renderMenu(id, data, callback) {
        ...
        (data || []).forEach(item => {
            ...
            card.innerHTML = `
                <span class="card-icon">${this.safeIcon(this.resolveMenuIcon(item, menuVariant), '\u{1F4DD}')}</span>
    ...
    renderLessonBlock(block) {
        ...
        if (block.type === 'paragraph') {
            return `<p class="lesson-block lesson-block--paragraph">${block.text || ''}</p>`;
        }

        if (block.type === 'example') {
            return `
                <section class="lesson-block lesson-block--example">
                    <div class="lesson-block-label">${block.label || 'Exemple'}</div>
                    <div class="lesson-block-content">${block.content || ''}</div>
                </section>
            `;
        }
        ...
        if (block.type === 'tip') {
            return `
                <section class="lesson-block lesson-block--tip">
                    <div class="lesson-block-label">${block.label || 'À retenir'}</div>
                    <div class="lesson-block-content">${block.content || ''}</div>
                </section>
            `;
        }
        ...
        if (block.type === 'bullets') {
            ...
            return `
                <section class="lesson-block lesson-block--bullets">
                    ${block.label ? `<div class="lesson-block-label">${block.label}</div>` : ''}
                    <ul class="lesson-bullets">${items.map((item) => `<li>${item}</li>`).join('')}</ul>
                </section>
            `;
        }
