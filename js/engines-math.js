const EnginesMath = {
    calculate(p) {
        ...
        switch (p.type) {
            ...
            case 'proportionnalite': {
                ...
                return { question: `<span class="small-question">${question}</span>`, answer: askPrix ? prix2 : qty2, inputType: 'numeric' };
            }
            case 'pourcentage': {
                ...
                return { question: `<span class="small-question">${pct} % de ${base} = ?</span>`, answer: result, inputType: 'numeric' };
            }
            case 'aire-rectangle': {
                const longueur = rnd(2, p.max || 12);
                const largeur = rnd(2, p.max || 12);
                return { question: `<span class="small-question">Un rectangle mesure ${longueur} cm de longueur et ${largeur} cm de largeur.<br>Quelle est son aire (en cm²) ?</span>`, answer: longueur * largeur, inputType: 'numeric' };
            }
            case 'volume-pave': {
                const longueur = rnd(2, p.max || 6);
                const largeur = rnd(2, p.max || 6);
                const hauteur = rnd(2, p.max || 6);
                return { question: `<span class="small-question">Un pavé droit mesure ${longueur} cm de longueur, ${largeur} cm de largeur et ${hauteur} cm de hauteur.<br>Quel est son volume (en cm³) ?</span>`, answer: longueur * largeur * hauteur, inputType: 'numeric' };
            }
            case 'echelle': {
                const echelle = pick(p.echelles || [50, 100, 200, 1000]);
                const planCm = rnd(1, 9);
                const reelM = (planCm * echelle) / 100;
                return { question: `<span class="small-question">Sur un plan à l'échelle 1/${echelle},<br>${planCm} cm représente combien de mètres dans la réalité ?</span>`, answer: reelM, inputType: 'numeric' };
            }
            case 'division-posed': {
    ...
    conversion(p) {
        ...
        if (p.subtype === 'metric') {
            ...
            return { question: "Convertis :", answer: result.toString().replace('.', ','), inputType: 'numeric', isVisual: true, visualType: 'conversionTable', data: { val, u1, u2, type: unitType } };
        }
